...
parser.add_argument("--data_path", type=str, default="/home/yons/huanghao/glass_dataset/GSD", help="")
...
parser.add_argument("--result_path", type=str, default="./vggt_s_v4_test", help="")
...
def main():
    # ######## create Model #############
    model = VGGT(enable_camera=False, enable_point=True, enable_depth=True, enable_track=False, enable_glass=True).cuda()
    apply_lora(model)
    model.load_state_dict(torch.load("./ckpt/vggt_s_v4_fdfp/vggt_s_basic.pth"), strict=False)
    ...
    image_name = "GDD_5203"
    image_path = f"E:/dataset/all_single_glass/test/image/{image_name}.jpg"
    image = Image.open(image_path)
    image = img_transform(image).unsqueeze(0).unsqueeze(0).cuda()
    with torch.no_grad():
        predictions = model(image)
        glass = predictions['glass'][0]
        edge = predictions['edge'][0]
        depth = predictions['depth']
        point = predictions['world_points']
        depth = depth[0].squeeze(0)
        depth = depth.squeeze(0).detach().cpu().numpy()
        glass = glass.squeeze(0)
        glass = torch.round(glass).squeeze(0).detach().int().cpu()
        edge = edge.squeeze(0)
        edge = torch.round(edge).squeeze(0).detach().int().cpu()
        point = point[0].squeeze(0)
        point = point.squeeze(0).detach().cpu().numpy()
        os.makedirs(os.path.join(opt.result_path, image_name), exist_ok=True)

        depth = np.nan_to_num(depth, nan=0.0, posinf=0.0, neginf=0.0)
        # 归一化到0~255
        depth_norm = cv2.normalize(depth, None, 0, 255, cv2.NORM_MINMAX)
        depth_uint8 = depth_norm.astype(np.uint8)
        cv2.imwrite(os.path.join(opt.result_path, image_name, 'depth.png'), depth_uint8)
        # np.save(os.path.join(opt.result_path, image_name, 'depth.npy'), depth)
        np.save(os.path.join(opt.result_path, image_name, 'point.npy'), point)
        g_final = np.array(transforms.Resize((opt.data_size, opt.data_size))(to_pil(glass)))
        cv2.imwrite(os.path.join(opt.result_path, image_name, 'glass.png'), g_final * 255.0)
        edge_final = np.array(transforms.Resize((opt.data_size, opt.data_size))(to_pil(edge)))
        cv2.imwrite(os.path.join(opt.result_path, image_name, 'edge.png'), edge_final * 255.0)
