...
class SimpleDecoder(nn.Module):
    def __init__(self):
        ...
        self.glass_upsample4 = UpGlass(512, 256)
        self.glass_upsample3 = UpGlass(256, 128)
        self.glass_upsample2 = UpGlass(128, 64)
        self.glass_upsample1 = UpGlass(64, 32)
        self.glass_upsample_final = UpGlass(32, 32)

        self.deconv4_glass = BasicConv(256, 256)
        self.deconv3_glass = BasicConv(128, 128)
        self.deconv2_glass = BasicConv(64, 64)
        self.deconv1_glass = BasicConv(32, 32)
        self.deconv_glass = BasicConv(32, 32)

        self.glass_head4 = nn.Conv2d(256, 1, 1)
        self.glass_head3 = nn.Conv2d(128, 1, 1)
        self.glass_head2 = nn.Conv2d(64, 1, 1)
        self.glass_head1 = nn.Conv2d(32, 1, 1)
        self.glass_pred_final = nn.Conv2d(32, 1, 3, 1, 1)

        self.edge_head4 = nn.Conv2d(256, 1, 3, 1, 1)
        self.edge_head3 = nn.Conv2d(128, 1, 3, 1, 1)
        self.edge_head2 = nn.Conv2d(64, 1, 3, 1, 1)
        self.edge_head1 = nn.Conv2d(32, 1, 3, 1, 1)
        self.edge_pred_final = nn.Conv2d(32, 1, 3, 1, 1)
    ...
    def forward(self, fea_list):
        ...
        glass4 = torch.sigmoid(self.glass_head4(d4_glass))
        edge4 = torch.sigmoid(self.edge_head4(d4_glass))
        ...
        glass3 = torch.sigmoid(self.glass_head3(d3_glass))
        edge3 = torch.sigmoid(self.edge_head3(d3_glass))
        ...
        glass2 = torch.sigmoid(self.glass_head2(d2_glass))
        edge2 = torch.sigmoid(self.edge_head2(d2_glass))

        feature_temp = self.glass_upsample1(d2_glass)
        d1_glass = self.deconv1_glass(feature_temp)
        glass1 = torch.sigmoid(self.glass_head1(d1_glass))
        edge1 = torch.sigmoid(self.edge_head1(d1_glass))

        feature_temp = self.glass_upsample_final(d1_glass)
        d_glass = self.deconv_glass(feature_temp)
        glass = torch.sigmoid(self.glass_pred_final(d_glass))
        edge = torch.sigmoid(self.edge_pred_final(d_glass))

        return d_glass, [glass, glass1, glass2, glass3, glass4], [edge, edge1, edge2, edge3, edge4]
...
class GLASSHead(nn.Module):
    ...
    def _forward_impl(
        ...
        # f_glass, glass_list, edge_list = self.decode(out, c_glass, c_edge, f_dpt, f_p3d)
        f_glass, glass_list, edge_list = self.decode(out)
...
class Decoder_block(nn.Module):
    def __init__(self, in_channel, out_channel, has_res=True):
        ...
        self.pred_glass = nn.Conv2d(3*out_channel, 1, kernel_size=1)
    ...
    def forward(self, fea_de, fea_en, fea_dep, fea_p3d, m_glass, m_edge):
        ...
        res_dep = res * w_dep
        res_p3d = res_high * w_p3d
        res_dep = self.cbam(self.out_conv1(torch.cat((res_dep, fea_dep), dim=1))*m_glass)
        res_p3d = self.rcab(self.out_conv2(torch.cat((res_p3d, fea_p3d), dim=1))*m_edge)
        edge = self.sigmoid(self.pred_edge(res_p3d))
        glass = self.sigmoid(self.pred_glass(torch.cat((res_dep, res_p3d, res), dim=1)))
        out = res_dep + res_p3d + res
...
class Decoder(nn.Module):
    ...
    def forward(self, fea_list, c_glass, c_edge, f_dep, f_p3d):
        ...
        return f_d, [glass0, glass1, glass2, glass3], [edge0, edge1, edge2, edge3]
...
class Frequency_Module(nn.Module):
    def __init__(self, dim_in, out_channels=[128, 256, 512, 1024]):
        ...
        self.atten = nn.ModuleList(
            [Attention(q_dim=oc, kv_dim=oc, embed_dim=oc) for oc in out_channels]
        )
    ...
    def forward(self, fea_list, patch_h, patch_w):
        ...
        for i in range(len(fea_list) - 1, -1, -1):
            ...
            temp_fea = self.atten[i](temp_fea, A)  # (B, N, D)
