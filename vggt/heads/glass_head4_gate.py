...
class Decoder_block(nn.Module):
    def __init__(self, in_channel, out_channel, has_res=True):
        ...
        self.gate = nn.Sequential(
            nn.Conv2d(out_channel * 3, 2, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(out_channel, 2, kernel_size=1)
        )
    ...
    def forward(self, fea_de, fea_en, fea_dep, fea_p3d, m_glass, m_edge):
        ...
        gates = self.gate(torch.cat((res_p3d, res_dep, fea_en), dim=1))  # (B, 3, H, W)
        gates = torch.tanh(gates)
        g1, g2 = gates[:, 0:1], gates[:, 1:2]
        f_fusion = g1 * res_p3d + g2 * res_dep + fea_en
