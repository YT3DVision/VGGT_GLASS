...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1):
    ...
    loss_reg = torch.norm(gt - pred, dim=-1)
...
def normal_loss(prediction, target, mask, cos_eps=1e-8, conf=None, gamma=1.0, alpha=0.2):
    ...
    pred_normals = torch.masked_select(pred_normals, all_valid[..., None].bool())
    pred_normals = pred_normals.view(-1, 3)

    gt_normals = torch.masked_select(gt_normals, all_valid[..., None].bool())
    gt_normals = gt_normals.view(-1, 3)
