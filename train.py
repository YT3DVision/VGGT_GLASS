...
from vggt.models.lora import apply_lora, save_lora
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default=r"E:/dataset/FP_VGD", help="")
    ...
    train_dataset = make_dataSet(opt.data_path, data_size=opt.data_size, sequence_length=opt.len_s, train=True)
    ...
    model.load_state_dict(torch.load("./pre_train/model.pt"), strict=False)
    # model.glass_head.load_state_dict(torch.load(os.path.join(opt.save_path, "validation_iou_max.pth")))
    apply_lora(model)
    train_params = []
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (glass_images, gt_images, video_name, index) in enumerate(train_iterator):
            # data_time = time.time()
            ...
            pred = predictions['glass'].reshape(b*s,c,h,w)
            gt_images = gt_images.reshape(b*s,c,h,w)
            loss = bce_ssim_loss(pred, gt_images)
            # print(loss)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i, (glass_images, gt_images, video_name, index) in enumerate(val_iterator):
                glass_images = glass_images.cuda()
                gt_images = gt_images.cuda()
        ...
        print(
            "INFO: epoch:{},train loss:{},validation iou:{}, time:{}".format(
                epoch + 1, train_loss_sum / len(train_loader), (v_glass_iou / (len(valid_loader)*3*opt.batchsize)) * 100,
                round(t, 2),
            )
        )
        ...
        if v_glass_iou > valid_max_glass:
            ...
            torch.save(model.glass_head.state_dict(), os.path.join(opt.save_path, "validation_iou_max.pth"))
